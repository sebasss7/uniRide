...
export default function ChatScreen() {
  ...
  const filterChats = useMemo(() => {
    const chatsPropios = chats.filter((chat) => {
    ...
    return chatsPropios.filter((chat) => {
      ...
      const otroUsuario =
        usersMock.find((user) => user.id === otroUsuarioId) ?? usersMock[0];

      return otroUsuario.nombre
        .toLowerCase()
        .includes(searchQuery.toLowerCase());
    });
  }, [chats, searchQuery, myId]);
  ...
  const renderChat = ({ item: chat }: { item: any }) => {
    ...
    //Buscar su info
    const otroUsuario =
      usersMock.find((u) => u.id === otroUsuarioId) || usersMock[0];

    //Buscar último mensaje
    const mensajesDelChat = mensajes.filter(
      (mensaje) => mensaje.chat_id === chat.id,
    );
